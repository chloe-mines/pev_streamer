...
# Text overlay streaming script - no rider icons
...
echo ""
echo "Starting ffmpeg stream with text overlay only..."
...
if [ "$AUDIO_INDEX" = "n" ]; then
  ...
  # Simple filter: overlay box, header, watermark, and text only
  if [ "$RECORD_STREAM" = "y" ]; then
    ffmpeg \
      ...
      -filter_complex "
        [2:v]scale=iw*1.0:ih*1.8[bg_wide];
        [bg_wide]format=rgba[bg];
        [0:v][bg]overlay=80:120[tmp1];
        [1:v]scale=60:60,format=rgba,colorchannelmixer=aa=0.5[esra_watermark];
        [1:v]scale=40:40,format=rgba[esra_header];
        [tmp1][esra_header]overlay=160:135[tmp2];
        [tmp2][esra_watermark]overlay=main_w-overlay_w-20:main_h-overlay_h-20[tmp3];
        [tmp3]drawtext=fontfile=/System/Library/Fonts/Supplemental/Helvetica.ttc:textfile=/tmp/laptimes.txt:reload=1:x=135:y=180:fontsize=16:fontcolor=white:shadowcolor=black:shadowx=2:shadowy=2:line_spacing=6:box=0[tmp4];
        [tmp4]split=2[out1][out2]
      " \
      -map "[out1]" \
      -c:v libx264 \
      -preset fast \
    ...
    ffmpeg \
      ...
      -filter_complex "
        ...
        [tmp1][esra_header]overlay=160:135[tmp2];
        [tmp2][esra_watermark]overlay=main_w-overlay_w-20:main_h-overlay_h-20[tmp3];
        [tmp3]drawtext=fontfile=/System/Library/Fonts/Supplemental/Helvetica.ttc:textfile=/tmp/laptimes.txt:reload=1:x=135:y=180:fontsize=16:fontcolor=white:shadowcolor=black:shadowx=2:shadowy=2:line_spacing=6:box=0
      " \
      -c:v libx264 \
      -preset fast \
      -profile:v baseline \
